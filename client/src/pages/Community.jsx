...
import {
  faClose,
  faHeart as faHeartSupported,
} from "@fortawesome/free-solid-svg-icons";
import {
  faHeart,
  faPaperPlane as faShareNodes,
...
const Community = () => {
  ...
  useEffect(() => {
    ...
    document.title = "Mindance - Community";
  ...
  const supportPost = (postId) => {
    if (supportedPostIds.includes(postId)) {
      ...
      fetch(
        `https://server-innercalm.vercel.app/api/notSupportPost?id=${postId}`,
        {
          method: "PUT",
        }
      )
        .then((response) => {
          if (response.status === 200) {
            ...
            const updatedSupportedPostIds = supportedPostIds.filter(
              (id) => id !== postId
            );
      ...
      fetch(
        `https://server-innercalm.vercel.app/api/supportPost?id=${postId}`,
        {
          method: "PUT",
        }
      )
  ...
  const FetchPosts = async () => {
    try {
      const resFromBack = await fetch(
        "https://server-innercalm.vercel.app/api/allPosts",
        {
          method: "GET",
          headers: {
    ...
    <>
      ...
      <div className="CMT">
        <div className={theme + " " + cmtpg}>
          ...
          <h1 className={theme + " head_st"} id="story">
            Stories of People
          </h1>
            ...
            {allposts.map((val) => {
              return (
                <div key={val._id} className={theme + " post"}>
                  ...
                  >
                    <p className={theme + " post_text"}>{val.story}</p>
                  </div>
                  <div
